...
Hooks automatically run commands in response to CodeAgent events. They're ideal for enforcement and automation that should happen consistently.
...
Notification hooks run when CodeAgent sends notifications. Use matchers to filter by notification type.
    ...
    "Notification": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"the agent is waiting\" with title \"CodeAgent\"'"
          }
        ]
      }
    ]
...
|---------|---------------|
| `permission_prompt` | the agent needs permission for a tool |
| `idle_prompt` | the agent waiting for input (60+ seconds) |
...
Hooks go in `.codeAgent/settings.json`:
...
Recommend creating the `.codeAgent/` directory if it doesn't exist.
