...
Skills are packaged expertise with workflows, reference materials, and best practices. Create them in `.codeAgent/skills/<name>/SKILL.md`. Skills can be invoked by the agent automatically when relevant, or by users directly with `/skill-name`.
...
Create project-specific skills in `.codeAgent/skills/<name>/SKILL.md`.
...
user-invocable: false           # Only the agent can invoke (for background knowledge)
...
| Setting | User | the agent | Use for |
|---------|------|--------|---------|
...
Generate OpenAPI documentation for the endpoint at 用户在命令后敲的参数（见本消息末尾原文）.
...
```
.codeAgent/skills/create-migration/
├── SKILL.md
└── scripts/
...
Create a migration for: 用户在命令后敲的参数（见本消息末尾原文）

1. Generate migration file in `migrations/` with timestamp prefix
2. Include up and down functions
3. Run validation: `bash ~/.codeAgent/skills/create-migration/scripts/validate-migration.sh`
...
Generate tests for: 用户在命令后敲的参数（见本消息末尾原文）
...
```
.codeAgent/skills/new-component/
├── SKILL.md
└── templates/
...
Create component: 用户在命令后敲的参数（见本消息末尾原文）
...
|---------|---------|---------|
| `用户在命令后敲的参数（见本消息末尾原文）` | All args as string | `/deploy staging` → "staging" |

Arguments are appended as `ARGUMENTS: <value>` if `用户在命令后敲的参数（见本消息末尾原文）` isn't in the skill.
