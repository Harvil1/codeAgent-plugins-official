...
description: Analyze a codebase and recommend CodeAgent automations (hooks, subagents, skills, plugins, MCP servers). Use when user asks for automation recommendations, wants to optimize their CodeAgent setup, mentions improving CodeAgent workflows, asks how to first set up CodeAgent for a project, or wants to know what CodeAgent features they should use.
tools: Read, Glob, Grep, Bash
---

# the agent Automation Recommender

Analyze codebase patterns to recommend tailored CodeAgent automations across all extensibility options.
...
# Check for existing CodeAgent config
ls -la .codeAgent/ CODEAGENT.md 2>/dev/null
...
Create skills in `~/.codeAgent/skills/<name>/SKILL.md`. Some are also available via plugins:
...
```markdown
## CodeAgent Automation Recommendations
...
**Install**: `the agent mcp add context7`
...
#### [skill name]
**Why**: [specific reason]
**Create**: `~/.codeAgent/skills/[name]/SKILL.md`
...
**Where**: `~/.codeAgent/agents/[name].md`
...
Recommend headless the agent for automated pipelines:

```bash
# Pre-commit hook example
uv run python main.py -p "fix lint errors in src/" --allowedTools Edit,Write

# CI pipeline with structured output
uv run python main.py -p "<prompt>" --output-format stream-json | your_command
```

### Permissions for Hooks

Configure allowed tools in `~/.codeAgent/settings.json`:

```json
{
  "permissions": {
    "allow": ["Edit", "Write", "terminal(npm test:*)", "terminal(git commit:*)"]
  }
}
```
