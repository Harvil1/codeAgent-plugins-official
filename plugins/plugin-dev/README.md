...
A comprehensive toolkit for developing CodeAgent plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality CodeAgent plugins:
...
7. **Testing** - Verify plugin works in CodeAgent
...
- Uses delegate_task-creator for AI-assisted agent generation
...
**Usage:**
```bash
/create-plugin [optional description]

# Examples:
/create-plugin
/create-plugin A plugin for managing database migrations
```
...
- AI-assisted agent generation using CodeAgent's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from CodeAgent), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for CodeAgent plugins
...
This keeps CodeAgent's context focused while providing deep knowledge when needed.
...
- Based on official CodeAgent documentation
...
- Use debug mode (`the agent --debug`)
