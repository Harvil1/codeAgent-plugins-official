...
**.codeAgent/experimental-features.local.md:**
...
# Read settings
FRONTMATTER=$(sed -n '/^---$/,/^---$/{ /^---$/d; p; }' ".codeAgent/my-plugin.local.md")
...
```bash
# Edit settings
vim .codeAgent/my-plugin.local.md
...
Changes require CodeAgent restart - hooks can't be hot-swapped.
