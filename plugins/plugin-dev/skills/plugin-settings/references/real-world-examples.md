...
Detailed analysis of how production plugins use the `.codeAgent/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".codeAgent/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.codeAgent/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".codeAgent/multi-agent-swarm.local.md" > temp.md
mv temp.md ".codeAgent/multi-agent-swarm.local.md"
```
...
Document any changes needed in CODEAGENT.md.
```
...
**Purpose:** Prevent session exit and loop the agent's output back as input
...
RALPH_STATE_FILE=".codeAgent/ralph-loop.local.md"
...
# Create state file
cat > ".codeAgent/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .codeAgent/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.codeAgent/multi-agent-swarm.local.md` | `.codeAgent/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.codeAgent/my-plugin.local.md"

# GOOD
FILE=".codeAgent/my-plugin.local.md"
```
...
The `.codeAgent/plugin-name.local.md` pattern provides:
