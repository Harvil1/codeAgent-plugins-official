...
description: This skill should be used when the user asks about "plugin settings", "store plugin configuration", "user-configurable plugin", ".local.md files", "plugin state files", "read YAML frontmatter", "per-project plugin settings", or wants to make plugin behavior configurable. Documents the .codeAgent/plugin-name.local.md pattern for storing plugin-specific configuration with YAML frontmatter and markdown content.
version: 0.1.0
---
...
# Plugin Settings Pattern for CodeAgent Plugins

## Overview

Plugins can store user-configurable settings and state in `.codeAgent/plugin-name.local.md` files within the project directory. This pattern uses YAML frontmatter for structured configuration and markdown content for prompts or additional context.

**Key characteristics:**
- File location: `.codeAgent/plugin-name.local.md` in project root
...
- Prompts to feed back to the agent
- Documentation or notes
```

### Example: Plugin State File

**.codeAgent/my-plugin.local.md:**
```markdown
---
enabled: true
strict_mode: false
...
STATE_FILE=".codeAgent/my-plugin.local.md"
...
Steps:
1. Check if settings exist at `.codeAgent/my-plugin.local.md`
2. Read configuration using read_file tool
...
Check for plugin settings at `.codeAgent/my-plugin.local.md`.
...
```bash
#!/bin/bash
STATE_FILE=".codeAgent/security-scan.local.md"
...
**.codeAgent/multi-agent-swarm.local.md:**
```markdown
---
...
2. Create `.codeAgent/my-plugin.local.md` with YAML frontmatter
3. Set appropriate values based on user input
4. Inform user that settings are saved
5. Remind user to restart CodeAgent for hooks to recognize changes
```
...
Create `.codeAgent/my-plugin.local.md` in your project:
...
After creating or editing, restart CodeAgent for changes to take effect.
```
...
✅ **DO:**
- Use `.codeAgent/plugin-name.local.md` format
- Match plugin name exactly
- Use `.local.md` suffix for user-local files

❌ **DON'T:**
- Use different directory (not `.codeAgent/`)
...
**Important:** Settings changes require CodeAgent restart.
...
After editing `.codeAgent/my-plugin.local.md`:
1. Save the file
2. Exit CodeAgent
...
```bash
if [[ ! -f ".codeAgent/my-plugin.local.md" ]]; then
...
3. Add gitignore entry for `.codeAgent/*.local.md`
...
7. Remind users that changes require CodeAgent restart
