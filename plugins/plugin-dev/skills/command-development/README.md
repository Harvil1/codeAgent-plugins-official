...
Comprehensive guidance on creating CodeAgent slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
- Dynamic arguments (用户在命令后敲的参数（见本消息末尾原文）, $1, $2, etc.)
- File references with @ syntax
- Bash execution with !` syntax
- Command organization and namespacing
- Best practices for command development
- Plugin-specific features (~/.codeAgent/plugins/<插件名>, plugin patterns)
...
- Dynamic arguments (用户在命令后敲的参数（见本消息末尾原文） and positional)
...
**Plugin-Specific:**
- ~/.codeAgent/plugins/<插件名> environment variable
...
- **plugin-features-reference.md**: Plugin-specific command features
  - Plugin command discovery and organization
  - ~/.codeAgent/plugins/<插件名> environment variable usage
  - Plugin command patterns (configuration, template, multi-script)
...
CodeAgent activates this skill when users:
...
the agent loads references and examples as needed based on task.
...
- Arguments: $1, $2, or 用户在命令后敲的参数（见本消息末尾原文）
- Files: @path/to/file
- Bash: !`command here`
```
...
- **Project**: `.codeAgent/commands/` (shared with team)
- **Personal**: `~/.codeAgent/commands/` (your commands)
- **Plugin**: `plugin-name/commands/` (plugin-specific)

### Key Features

**Dynamic arguments:**
- `用户在命令后敲的参数（见本消息末尾原文）` - All arguments as single string
...
| `allowed-tools` | Restrict tool access | `Read, terminal(git:*)` |
...
- ✓ Plugin command patterns (~/.codeAgent/plugins/<插件名>, discovery, organization)
...
6. Test examples work with current CodeAgent
