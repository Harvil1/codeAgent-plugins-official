...
Practical examples of commands designed for CodeAgent plugins, demonstrating plugin-specific patterns and features.
...
allowed-tools: terminal(node:*), Read
---

Analyze @$1 using plugin's quality checker:

!`node ~/.codeAgent/plugins/<插件名>/scripts/quality-check.js $1`
...
**Key features:**
- Uses `~/.codeAgent/plugins/<插件名>` for portable path
...
**Security scan:**
!`bash ~/.codeAgent/plugins/<插件名>/scripts/security-scan.sh $1`

**Performance analysis:**
!`bash ~/.codeAgent/plugins/<插件名>/scripts/perf-analyze.sh $1`

**Best practices check:**
!`bash ~/.codeAgent/plugins/<插件名>/scripts/best-practices.sh $1`
...
Template structure: @~/.codeAgent/plugins/<插件名>/templates/api-documentation.md
...
allowed-tools: terminal(*), Read
---

Executing release workflow for version $1:

**Step 1 - Pre-release validation:**
!`bash ~/.codeAgent/plugins/<插件名>/scripts/pre-release-check.sh $1`

**Step 2 - Build artifacts:**
!`bash ~/.codeAgent/plugins/<插件名>/scripts/build-release.sh $1`

**Step 3 - Run test suite:**
!`bash ~/.codeAgent/plugins/<插件名>/scripts/run-tests.sh`

**Step 4 - Package release:**
!`bash ~/.codeAgent/plugins/<插件名>/scripts/package.sh $1`
...
Deployment configuration for $1: @~/.codeAgent/plugins/<插件名>/config/$1-deploy.json
...
- Plugin's linting rules: ~/.codeAgent/plugins/<插件名>/config/lint-rules.json
- Security checklist: ~/.codeAgent/plugins/<插件名>/checklists/security.md
- Performance guidelines: ~/.codeAgent/plugins/<插件名>/docs/performance.md

Note: This uses the delegate_task tool to launch the plugin's code-reviewer agent for thorough analysis.
```
...
Run plugin analyzer: !`node ~/.codeAgent/plugins/<插件名>/scripts/analyze.js $1`
...
Template: @~/.codeAgent/plugins/<插件名>/templates/review-report.md
...
Check build script exists: !`test -x ~/.codeAgent/plugins/<插件名>/scripts/build.sh && echo "EXISTS" || echo "MISSING"`

Verify configuration available: !`test -f ~/.codeAgent/plugins/<插件名>/config/$1.json && echo "FOUND" || echo "NOT_FOUND"`

If all validations pass:

**Configuration:** @~/.codeAgent/plugins/<插件名>/config/$1.json

**Execute build:** !`bash ~/.codeAgent/plugins/<插件名>/scripts/build.sh $1 2>&1`

**Validation results:** !`bash ~/.codeAgent/plugins/<插件名>/scripts/validate-build.sh $1 2>&1`
...
Load environment configuration: @~/.codeAgent/plugins/<插件名>/config/$1-checks.json

Determine check level: !`echo "$1" | grep -E "^prod$" && echo "FULL" || echo "BASIC"`

**For production environment:**
- Full test suite: !`bash ~/.codeAgent/plugins/<插件名>/scripts/test-full.sh`
- Security scan: !`bash ~/.codeAgent/plugins/<插件名>/scripts/security-scan.sh`
- Performance audit: !`bash ~/.codeAgent/plugins/<插件名>/scripts/perf-check.sh`
- Compliance check: !`bash ~/.codeAgent/plugins/<插件名>/scripts/compliance.sh`

**For non-production environments:**
- Basic tests: !`bash ~/.codeAgent/plugins/<插件名>/scripts/test-basic.sh`
- Quick lint: !`bash ~/.codeAgent/plugins/<插件名>/scripts/lint.sh`
...
```markdown
Check exists: !`test -f ~/.codeAgent/plugins/<插件名>/path/file && echo "YES" || echo "NO"`
```
...
1. **Test with plugin installed:**
   ```bash
   cd /path/to/plugin
   uv run python main.py -c  # /command-name args
   ```

2. **Verify ~/.codeAgent/plugins/<插件名> expansion:**
   ```bash
   # Add debug output to command
   !`echo "Plugin root: ~/.codeAgent/plugins/<插件名>"`
   ```

3. **Test across different working directories:**
   ```bash
   cd /tmp && uv run python main.py -c  # /command-name
   cd /other/project && uv run python main.py -c  # /command-name
   ```

4. **Validate resource availability:**
   ```bash
   # Check all plugin resources exist
   !`ls -la ~/.codeAgent/plugins/<插件名>/scripts/`
   !`ls -la ~/.codeAgent/plugins/<插件名>/config/`
   ```

### Common Mistakes to Avoid

1. **Using relative paths instead of ~/.codeAgent/plugins/<插件名>:**
   ```markdown
   # Wrong
   !`node ./scripts/analyze.js`

   # Correct
   !`node ~/.codeAgent/plugins/<插件名>/scripts/analyze.js`
   ```
...
2. **Forgetting to allow required tools:**
   ...
   allowed-tools: terminal(*)
   ---
   !`bash ~/.codeAgent/plugins/<插件名>/scripts/script.sh`
   ```
...
4. **Hardcoding plugin paths:**
   ...
   @/home/user/.codeAgent/plugins/my-plugin/config.json

   # Correct - works everywhere
   @~/.codeAgent/plugins/<插件名>/config.json
   ```
