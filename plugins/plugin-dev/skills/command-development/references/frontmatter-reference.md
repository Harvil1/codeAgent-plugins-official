...
```yaml
allowed-tools: terminal(git:*)           # Only git commands
allowed-tools: terminal(npm:*)           # Only npm commands
allowed-tools: terminal(docker:*)        # Only docker commands
```
...
3. **Bash execution:** Enable bash command output
   ```yaml
   allowed-tools: terminal(git status:*), terminal(git diff:*)
   ```
...
**Purpose:** Specify which the agent model executes the command
...
**Best practices:**
- Use sparingly (limits the agent's autonomy)
...
argument-hint: [app-name] [environment] [version]
allowed-tools: terminal(kubectl:*), terminal(helm:*), Read
model: sonnet
---
...
**Fix:** Use `terminal(git:*)` format

**Invalid model name:**
```yaml
model: gpt4  # ❌ Not a valid the agent model
```
