...
Comprehensive guide to creating commands that gather user feedback and make decisions through the ask_user tool.
...
For these cases, use the **ask_user tool** within command execution rather than relying on command arguments.
...
Based on the answers received from ask_user:
...
Create `.codeAgent/plugin-name.local.md` with:
...
allowed-tools: ask_user, Read, Write, Bash
---
...
Use ask_user to ask about deployment basics.
...
Use ask_user for final confirmation:
- header: "Confirm"
...
```markdown
Use ask_user for enabling features:
...
## Command Patterns with ask_user
...
Use ask_user with multiple questions in one call:
...
allowed-tools: ask_user, Read, Write
---
...
Use ask_user for member details:
...
After calling ask_user, verify answers received:
...
Use ask_user to collect settings.
...
allowed-tools: ask_user, Write, Read
---
...
Use ask_user for core settings.

Save to `.codeAgent/config-partial.yml`
...
Use ask_user for settings based on Phase 1 answers.
...
allowed-tools: ask_user, Bash, Read
---
...
Question: "What should we call delegate_task [number]?"
Header: "Agent name"
Options:
  - auth-agent
  - api-agent
...
Use ask_user for options that require explanation.
```
...
- Verify ask_user in allowed-tools
...
With ask_user, commands become interactive wizards that guide users through complex decisions while maintaining the clarity that simple arguments provide for straightforward inputs.
