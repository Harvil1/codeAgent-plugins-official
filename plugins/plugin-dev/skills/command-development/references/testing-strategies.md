...
head -n 20 .codeAgent/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .codeAgent/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .codeAgent/commands/*.md

# Check file is in correct location
test -f .codeAgent/commands/my-command.md && echo "Found" || echo "Missing"
```
...
# 5. Check debug logs
tail -f ~/.codeAgent/debug-logs/latest
...
- 用户在命令后敲的参数（见本消息末尾原文） captures all arguments
...
cat > .codeAgent/commands/test-bash.md << 'EOF'
---
description: Test bash execution
allowed-tools: terminal(echo:*), terminal(date:*)
---
...
cat > .codeAgent/commands/test-forbidden.md << 'EOF'
---
description: Test forbidden command
allowed-tools: terminal(echo:*)
---
...
# Command: .codeAgent/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.codeAgent/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .codeAgent/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .codeAgent/commands/; then
...
```bash
# Monitor CodeAgent during command execution
# In terminal 1:
uv run python main.py --debug

# In terminal 2:
watch -n 1 'ps aux | grep the agent'
...
```bash
# Check file location
ls -la .codeAgent/commands/my-command.md

# Check permissions
chmod 644 .codeAgent/commands/my-command.md

# Check syntax
head -n 20 .codeAgent/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .codeAgent/commands/my-command.md
grep '\用户在命令后敲的参数（见本消息末尾原文）' .codeAgent/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .codeAgent/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .codeAgent/commands/my-command.md

# Verify command syntax
grep '!\`' .codeAgent/commands/my-command.md
