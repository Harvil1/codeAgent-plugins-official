...
This reference covers features and patterns specific to commands bundled in CodeAgent plugins.
...
CodeAgent automatically discovers commands in plugins using the following locations:
...
`~/.codeAgent/plugins/<插件名>` is a special environment variable available in plugin commands that resolves to the absolute path of the plugin directory.
...
Run analysis: !`node ~/.codeAgent/plugins/<插件名>/scripts/analyze.js`

Read template: @~/.codeAgent/plugins/<插件名>/templates/report.md
```
...
Lint results: !`node ~/.codeAgent/plugins/<插件名>/bin/lint.js $1`
...
Configuration: @~/.codeAgent/plugins/<插件名>/config/deploy-config.json
...
Use this template: @~/.codeAgent/plugins/<插件名>/templates/api-report.md
...
Step 1 - Prepare: !`bash ~/.codeAgent/plugins/<插件名>/scripts/prepare.sh $1`
Step 2 - Config: @~/.codeAgent/plugins/<插件名>/config/$1.json
Step 3 - Execute: !`~/.codeAgent/plugins/<插件名>/bin/execute $1`
...
1. **Always use for plugin-internal paths:**
   ```markdown
   # Good
   @~/.codeAgent/plugins/<插件名>/templates/foo.md
...
2. **Validate file existence:**
   ...
   allowed-tools: terminal(test:*), Read
   ---

   !`test -f ~/.codeAgent/plugins/<插件名>/config.json && echo "exists" || echo "missing"`

   If config exists, load it: @~/.codeAgent/plugins/<插件名>/config.json
...
4. **Combine with arguments:**
   ```markdown
   Run: !`~/.codeAgent/plugins/<插件名>/bin/process.sh $1 $2`
   ```
...
- Verify syntax is exact: `~/.codeAgent/plugins/<插件名>`
...
Load configuration: @~/.codeAgent/plugins/<插件名>/deploy-config.json
...
Template: @~/.codeAgent/plugins/<插件名>/templates/component-docs.md
...
Validate: !`bash ~/.codeAgent/plugins/<插件名>/scripts/validate.sh`
...
Environment config: @~/.codeAgent/plugins/<插件名>/config/$1.json
...
Cache directory: ~/.codeAgent/plugins/<插件名>/cache/

Analyze @$1 and save results to cache:
!`mkdir -p ~/.codeAgent/plugins/<插件名>/cache && date > ~/.codeAgent/plugins/<插件名>/cache/last-run.txt`
...
Commands can trigger plugin agents using the delegate_task tool:
...
Note: This uses the delegate_task tool to launch the plugin's code-analyzer agent.
```

**Key points:**
- Agent must be defined in plugin's `agents/` directory
- the agent will automatically use delegate_task tool to launch agent
...
- Mention skill by name to hint the agent should invoke it
...
1. Bash validation using search_files/test
...
- Config exists: !`test -f ~/.codeAgent/plugins/<插件名>/config.json && echo "✓" || echo "✗"`
- Scripts exist: !`test -d ~/.codeAgent/plugins/<插件名>/scripts && echo "✓" || echo "✗"`
- Tools available: !`test -x ~/.codeAgent/plugins/<插件名>/bin/analyze && echo "✓" || echo "✗"`
...
Try processing: !`node ~/.codeAgent/plugins/<插件名>/scripts/process.js $1 2>&1 || echo "ERROR: $?"`
...
1. **Use ~/.codeAgent/plugins/<插件名> for all plugin-internal paths**
