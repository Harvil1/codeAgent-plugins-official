...
if [ ! -f ".codeAgent/command-initialized" ]; then
...
Checking for user config: .codeAgent/plugin-name.local.md

if [ -f ".codeAgent/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .codeAgent/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .codeAgent/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .codeAgent/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .codeAgent/plugin-name.local.md to customize"
fi
...
Or set in .codeAgent/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".codeAgent/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .codeAgent/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .codeAgent/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .codeAgent/operation-completed.flag
```
