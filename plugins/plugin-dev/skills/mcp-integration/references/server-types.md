...
Complete reference for all MCP server types supported in CodeAgent plugins.
...
{
  "my-server": {
    "command": "~/.codeAgent/plugins/<插件名>/servers/custom-server",
...
1. **Startup**: CodeAgent spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire CodeAgent session
4. **Shutdown**: Process terminated when CodeAgent exits
...
{
  "custom": {
    "command": "~/.codeAgent/plugins/<插件名>/servers/my-server.js",
    "args": ["--verbose"]
  }
}
```
...
1. **Use absolute paths or ~/.codeAgent/plugins/<插件名>**
