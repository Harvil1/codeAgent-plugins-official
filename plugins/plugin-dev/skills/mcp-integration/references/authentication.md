...
Complete guide to authentication methods for MCP servers in CodeAgent plugins.
...
CodeAgent automatically handles the complete OAuth 2.0 flow for SSE and HTTP servers:

1. User attempts to use MCP tool
2. CodeAgent detects authentication needed
3. Opens browser for OAuth consent
4. User authorizes in browser
5. Tokens stored securely by CodeAgent
...
No additional auth configuration needed! CodeAgent handles everything.
...
Tokens are stored securely by CodeAgent:
...
\`\`\`bash
export DATABASE_URL="postgresql://port/database"
...
```json
{
  "api": {
    ...
    "headersHelper": "~/.codeAgent/plugins/<插件名>/scripts/get-headers.sh"
  }
}
```
...
{
  "secure-api": {
    "command": "~/.codeAgent/plugins/<插件名>/servers/mtls-wrapper",
...
```json
{
  "headersHelper": "~/.codeAgent/plugins/<插件名>/scripts/generate-jwt.sh"
}
```
