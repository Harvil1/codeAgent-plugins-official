...
Complete guide to using MCP tools effectively in CodeAgent plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in CodeAgent tools.
...
- Can use any tool the agent determines is necessary
...
the agent automatically structures tool calls based on schema:

```typescript
// the agent generates this internally
{
...
```markdown
Steps:
1. Make parallel calls (the agent handles this automatically):
...
5. **Check debug output**: `the agent --debug`
