...
Use this template to generate agents using the agent with the agent creation system prompt.
...
Send this to the agent (with the agent-creation-system-prompt loaded):
...
Tell the agent when the agent should activate:
...
- **Comprehensive**: the agent includes edge cases and quality checks
