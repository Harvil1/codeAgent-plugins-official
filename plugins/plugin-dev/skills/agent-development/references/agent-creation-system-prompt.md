...
**You send to the agent with the system prompt above:**
```
Create an agent configuration based on this request: "I need an agent that reviews pull requests for code quality issues"
```

**the agent returns JSON (note: prose `whenToUse`, "When to invoke" section in `systemPrompt`):**
```json
{
...
- Follow project documentation standards from CODEAGENT.md
```
...
The prompt specifically mentions using CODEAGENT.md context:
...
2. Feed to the agent with this system prompt
