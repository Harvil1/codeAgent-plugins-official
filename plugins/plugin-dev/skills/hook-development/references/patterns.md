...
This reference provides common, proven patterns for implementing CodeAgent hooks. Use these patterns as starting points for typical hook use cases.
  ...
  "Notification": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ~/.codeAgent/plugins/<插件名>/scripts/log-notification.sh"
        }
      ]
    }
  ]
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ~/.codeAgent/plugins/<插件名>/scripts/check-quality.sh"
        }
      ]
    }
  ]
...
**Note:** Must restart CodeAgent after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE="$CLAUDE_PROJECT_DIR/.codeAgent/my-plugin.local.json"
...
**Configuration file (.codeAgent/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,
