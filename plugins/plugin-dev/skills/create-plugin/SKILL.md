...
<!-- CodeAgent 适配注：本项目插件布局 = plugin.json（或 .claude-plugin/plugin.json）+ skills/<名>/SKILL.md；旧式 commands/*.md 已自动转换成 skills；插件级 hooks/commands 暂不随插件加载；市场是自建 Gitee 镜像，装插件用 /plugin install。 -->
# Plugin Creation Workflow

Guide the user through creating a complete, high-quality CodeAgent plugin from initial concept to tested implementation. Follow a systematic approach: understand requirements, design components, clarify details, implement following best practices, validate, and test.

## Core Principles

- **Ask clarifying questions**: Identify all ambiguities about plugin purpose, triggering, scope, and components. Ask specific, concrete questions rather than making assumptions. Wait for user answers before proceeding with implementation.
- **Load relevant skills**: Use the load_skill tool to load plugin-dev skills when needed (plugin-structure, hook-development, agent-development, etc.)
...
- **Use task_create**: Track all progress throughout all phases

**Initial request:** 用户在命令后敲的参数（见本消息末尾原文）
...
**MUST load plugin-structure skill** using load_skill tool before this phase.
...
3. Create directory structure using terminal:
   ```bash
   ...
   ```
4. Create plugin.json manifest using write_file tool:
   ```json
   {
     "name": "plugin-name",
     "version": "0.1.0",
...
6. Create .gitignore if needed (for .codeAgent/\*.local.md, etc.)
...
1. Load skill-development skill using load_skill tool
2. For each skill:
   ...
   - For user-invoked skills (slash commands): include `description`, `argument-hint`, and `allowed-tools` frontmatter; write instructions FOR the agent (not TO user)
   - Create reference files for detailed content
   - Create example files for working code
   - Create utility scripts if needed
3. Use load_skill-reviewer agent to validate each skill
...
1. Load command-development skill using load_skill tool
2. For each command:
...
1. Load agent-development skill using load_skill tool
2. For each agent, use delegate_task-creator agent:
...
1. Load hook-development skill using load_skill tool
2. For each hook:
   - Create hooks/hooks.json with hook configuration
   - Prefer prompt-based hooks for complex logic
   - Use ~/.codeAgent/plugins/<插件名> for portability
   - Create hook scripts if needed (in examples/ not scripts/)
   - Test with validate-hook-schema.sh and test-hook.sh utilities

### For MCP:

1. Load mcp-integration skill using load_skill tool
2. Create .mcp.json configuration with:
   - Server type (stdio for local, SSE for hosted)
   - Command and args (with ~/.codeAgent/plugins/<插件名>)
...
1. Load plugin-settings skill using load_skill tool
2. Create settings template in README
3. Create example .codeAgent/plugin-name.local.md file (as documentation)
4. Implement settings reading in hooks/commands as needed
5. Add to .gitignore: `.codeAgent/*.local.md`
...
3. **Review with skill-reviewer** (if plugin has skills):
   - For each skill, use load_skill-reviewer agent
...
5. **Test hook configuration** (if plugin has hooks):
   ...
   - Verify ~/.codeAgent/plugins/<插件名> usage
...
**Goal**: Test that plugin works correctly in CodeAgent
...
3. **Testing recommendations**:
   ...
   - For user-invoked skills: Run `/skill-name` with various arguments
   - For agents: Create scenarios matching agent examples
   - For hooks: Use `the agent --debug` to see hook execution
...
- **Use task_create** to track progress at every phase
- **Load skills with load_skill tool** when working on specific component types
...
- **Apply best practices**:
  ...
  - Skill instructions written FOR the agent (not TO user)
  - Strong trigger phrases
  - ~/.codeAgent/plugins/<插件名> for portability
