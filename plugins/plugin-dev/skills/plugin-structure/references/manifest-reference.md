...
The manifest MUST be in the `.claude-plugin/` directory at the plugin root. CodeAgent will not recognize plugins without this file in the correct location.
...
- Plugin identification in CodeAgent
...
```json
{
  "mcpServers": {
    "github": {
      "command": "node",
      "args": ["~/.codeAgent/plugins/<插件名>/servers/github-mcp.js"],
      "env": {
...
When CodeAgent loads components:
...
CodeAgent validates the manifest on plugin load:
