...
4. **Register components**: Make available to CodeAgent
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during CodeAgent initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → CodeAgent looks up → Executes
**Agents**: Task arrives → CodeAgent evaluates capabilities → Selects agent
**Skills**: Task context matches description → CodeAgent loads skill
**Hooks**: Event occurs → CodeAgent calls matching hooks
...
**Note**: CodeAgent doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, CodeAgent doesn't support file references.
...
```bash
#!/bin/bash
source "~/.codeAgent/plugins/<插件名>/lib/test-utils.sh"
run_tests
```
