...
```json
{
  "mcpServers": {
    "kubernetes": {
      "command": "node",
      "args": ["~/.codeAgent/plugins/<插件名>/servers/kubernetes-mcp/index.js"],
      "env": {
    ...
    "terraform": {
      "command": "python",
      "args": ["~/.codeAgent/plugins/<插件名>/servers/terraform-mcp/main.py"],
      "env": {
    ...
    "github-actions": {
      "command": "node",
      "args": ["~/.codeAgent/plugins/<插件名>/servers/github-actions-mcp/server.js"],
      "env": {
...
\`\`\`javascript
const { DatadogClient } = require('~/.codeAgent/plugins/<插件名>/lib/integrations/datadog')
...
\`\`\`javascript
const { SlackClient } = require('~/.codeAgent/plugins/<插件名>/lib/integrations/slack')
...
- Use read_file-only root filesystem
...
\`\`\`bash
bash ~/.codeAgent/plugins/<插件名>/skills/kubernetes-ops/scripts/validate-manifest.sh deployment.yaml
\`\`\`
```
  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ~/.codeAgent/plugins/<插件名>/hooks/scripts/security/scan-secrets.sh",
          "timeout": 30
        }
      ]
    },
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ~/.codeAgent/plugins/<插件名>/hooks/scripts/workflow/update-status.sh",
          "timeout": 15
        }
      ]
    }
  ...
  "Stop": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ~/.codeAgent/plugins/<插件名>/hooks/scripts/quality/check-config.sh",
          "timeout": 45
        },
        {
          "type": "command",
          "command": "bash ~/.codeAgent/plugins/<插件名>/hooks/scripts/workflow/notify-team.sh",
          "timeout": 30
        }
      ]
    }
  ...
  "SessionStart": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ~/.codeAgent/plugins/<插件名>/hooks/scripts/security/validate-permissions.sh",
          "timeout": 20
        }
      ]
    }
