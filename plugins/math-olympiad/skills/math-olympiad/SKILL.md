...
**The delegate_task tool cannot enforce tool restriction.** Subagents get the full tool
set. The only mechanism is the prompt. Use this prompt VERBATIM — do not
summarize, do not synthesize your own:

```
NO COMPUTATION. Do not use terminal, Python, WebSearch, Read, Write, or any tool that runs code or fetches data. Numerical verification is not a proof step. "I computed n=1..10 and the pattern holds" is not a proof.
...
**NO WEB. NO LOOKUP.** Deep mode may use terminal/Python for bounded computation,
but NEVER web_fetch, WebSearch, or any network access. Finding the solution on
...
- The instruction: "NO WEB ACCESS — do not look up this problem or its solution.
  ...
  identity checks via terminal/Python only). 60-second computation limit. If n≤10
