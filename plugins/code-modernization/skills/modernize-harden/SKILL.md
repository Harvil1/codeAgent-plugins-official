...
(referred to as `$1` below) is the first non-flag token in `用户在命令后敲的参数（见本消息末尾原文）`;
...
```
Workflow({
  scriptPath: "~/.codeAgent/plugins/<插件名>/workflows/harden-scan.js",
  args: { system: "$1" }
})
```
...
**Fallback — direct subagent** (older CodeAgent builds without the
