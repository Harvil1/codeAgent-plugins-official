...
```
Workflow({
  scriptPath: "~/.codeAgent/plugins/<插件名>/workflows/uplift-deltas.js",
...
```
Workflow({
  scriptPath: "~/.codeAgent/plugins/<插件名>/workflows/uplift-migrate.js",
...
`str_replace`/`write_file` on `legacy/`); a shell command that writes a file goes
