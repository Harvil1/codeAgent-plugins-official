...
The first token of `用户在命令后敲的参数（见本消息末尾原文）` is the system dir (`$1`); **everything
...
```
Workflow({
  scriptPath: "~/.codeAgent/plugins/<插件名>/workflows/reimagine-scaffold.js",
...
Write `modernized/$1-reimagined/CODEAGENT.md` — the persistent context file for
