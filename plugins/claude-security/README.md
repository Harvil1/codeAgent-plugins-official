# CodeAgent Security Plugin for CodeAgent
...
This is the in-your-session version of [CodeAgent Security](https://claude.com/product/claude-security), Anthropic’s hosted product for vulnerability detection and patching. It runs entirely inside your CodeAgent session — no separate process, no daemon.

## the agent Fable 5.1 Support

The CodeAgent Security Plugin for CodeAgent supports the agent Fable 5.1, and it is the best model to use for discovering vulnerabilities. Portions of a scan may occasionally be downgraded to Opus 4.8; the rest of the scan completes on Fable 5.1. Please share feedback with `/feedback` so we can keep improving our cybersecurity safeguards.

## Where it runs

A scan and a fix both run in your CodeAgent session, under your permissions. The plugin reads the repository you have open the same way you would, and adds no isolation of its own: the directory's `.git/config`, its `.claude/` settings and hooks, and its `CODEAGENT.md` all apply exactly as they would in any other session.
...
If CodeAgent reports that the marketplace is not found, run `/plugin marketplace add anthropics/claude-plugins-official` first, then retry.
...
Throughout, what the repository says is evidence rather than instruction. Code, comments, and any `CODEAGENT.md` in the tree are read as data under review, so text addressed to the scan is noted rather than obeyed. Under the trusted-code model this keeps the work anchored to the evidence; it is not a defense against a hostile repository.

Scans are nondeterministic. Two scans of the same code can surface different findings, and the same scan finds more over time as models improve; running scans regularly builds coverage. CodeAgent Security reasons about code the way a human security researcher does, which complements SAST, dependency scanning, and code review rather than replacing them.
...
The plugin reports usage counts (scans started and finished, findings by severity, patches drafted, and which step failed when one does) through CodeAgent's built-in telemetry. To turn this off, use CodeAgent's own settings: set `DISABLE_TELEMETRY=1` (or `DO_NOT_TRACK=1`, or `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`) and these counts are not sent to Anthropic. See [CodeAgent's data usage documentation](https://code.claude.com/docs/en/data-usage#telemetry-services).
