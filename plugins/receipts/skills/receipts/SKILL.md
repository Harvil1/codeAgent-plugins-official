---
name: receipts
description: Generate a personal CodeAgent usage & impact report ("receipts") from this machine's local session transcripts — for justifying CodeAgent usage/spend to a manager, self-review, or "what have I been using this for" check-ins. Mines ~/.codeAgent/projects locally (no extra API calls beyond one final write-up), cross-references local git history, and writes a markdown report plus a self-contained HTML receipt to your home directory. Use when the user asks for "receipts", an "impact report", "usage report", wants to "show my CodeAgent activity", "prove the value of CodeAgent", or runs `/receipts`.
---

# /receipts — personal CodeAgent impact report

Generates a markdown report of one developer's own CodeAgent activity,
built entirely from local data:

- **Source data**: this machine's session transcripts at `~/.codeAgent/projects/**/*.jsonl`
...
Everything here is scoped to **work done with CodeAgent**, mapped to the
project it was done on. Two rules follow from that, and they explain most of
the shapes below:

- **CodeAgent's own machinery is not the dev's work.** The agent's
  scratchpad, its per-session tool output, and `~/.codeAgent` are excluded. Files
  the agent wrote to talk to itself are not files the dev shipped.
...
### Header
If `userName` is set, lead with it (e.g. "# Ada Lovelace's CodeAgent Receipt"
...
- `totals.commitsWithOurWork` as "commits carrying work CodeAgent did". The
  ...
  "commits made by CodeAgent"**: the dev may well have committed by hand.
  Qualify with `totals.gitActiveDayOverlap`: "N of your M active days ended
  with that work being committed."
- `prCreateCmds` as "PRs opened via CodeAgent" (only if > 0) — note this
