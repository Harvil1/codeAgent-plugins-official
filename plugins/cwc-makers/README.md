...
Plug in your M5Stack Cardputer-Adv over USB-C, type `/maker-setup`, and the agent will:

1. Clone [`moremas/build-with-claude`](https://github.com/moremas/build-with-claude)
2. Detect the device, flash UIFlow 2.0 firmware, and install the the agent Buddy + Hello + Snake app bundle
3. Walk you through the one physical step (the download-mode button press on the back of the device)
4. Hand you a working pocket computer that pairs with the agent Desktop over BLE

Then ask the agent to build whatever you want next — a magic 8-ball, a pixel pet, a weather ticker — and it'll write the MicroPython and push it to the device without re-flashing.
...
`/maker-setup` is the intended entry point; the skills are also auto-triggered by the agent when relevant. Skill content is vendored from the upstream repo so the agent has the domain knowledge in-context without symlinking anything into `~/.codeAgent/skills/`.
