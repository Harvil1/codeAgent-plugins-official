...
The message appears in chat and the agent responds to it. Use `role: "user"` — the widget speaks on the user's behalf.

### `app.updateModelContext({ content })`

Update the agent's context **silently** — no visible message. Use for state that informs but doesn't warrant a chat bubble.
...
Call a tool on your MCP server directly, bypassing the agent. Returns the tool result.
...
Use for data fetches that don't need the agent's reasoning — pagination, detail lookups, refreshes.
...
Fires with the arguments the agent passed to the tool. Useful if the widget needs to know what was asked for (e.g., highlight the search term).

### `app.ontoolinputpartial = ({ arguments }) => {...}` / `app.ontoolcancelled = () => {...}`

`ontoolinputpartial` fires while the agent is still streaming arguments — use it to show a skeleton ("Preparing: <title>…") before the result lands. `ontoolcancelled` fires if the call is aborted; clear the skeleton.
...
1. the agent calls a tool with `_meta.ui.resourceUri` declared
