...
Pre-flight before submitting a remote MCP app to the the agent connector
...
| **Helper tools** | Widget-only tools (geometry/image fetchers) carry `_meta.ui.visibility: ["app"]` so they don't appear in the agent's tool list. |
