...
One line of config, lands directly in the agent's system prompt. Use it for tool-use hints that don't fit in individual tool descriptions.
...
This is the highest-leverage one-liner in the spec. If the agent keeps misusing your tools, put the fix here.
...
**Requires client support** — check `clientCapabilities.sampling` first. Model preference hints are substring-matched (`"claude-3-5"` matches any the agent 3.5 variant).
...
| Elicitation | — | `elicitation: {}` | return text, ask the agent to relay |
