...
| Host | Status |
|---|---|
| CodeAgent | ✅ since v2.1.76 (both `form` and `url` modes) |
| the agent Desktop | Unconfirmed — likely not yet or very recent |
| claude.ai | Unknown |
...
server.registerTool("delete_all", {
  ...
  // Fallback: return text asking the agent to relay the question
