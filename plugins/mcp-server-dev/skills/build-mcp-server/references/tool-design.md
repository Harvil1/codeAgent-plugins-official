# Tool Design — Writing Tools the agent Uses Correctly

Tool schemas and descriptions are prompt engineering. They land directly in the agent's context and determine whether the agent picks the right tool with the right arguments. Most MCP integration bugs trace back to vague descriptions or loose schemas.
...
- Every tool **must** include `readOnlyHint`, `destructiveHint`, and `title` annotations — these determine auto-permissions in the agent.
...
- Tool descriptions **must not** instruct the agent how to behave (e.g. "always do X", "you must call Y first", overriding system instructions, promoting products) — treated as prompt injection at review.
...
**The description is the contract.** It's the only thing the agent reads before deciding whether to call the tool. Write it like a one-line manpage entry plus disambiguating hints.
...
the agent will call this for anything vaguely search-shaped, including things it can't do.
...
**Describe every parameter.** The `.describe()` text shows up in the schema the agent sees. Omitting it is leaving money on the table.
...
- Return bare success with no identifier (`"ok"` after a create — the agent can't reference what it made)
...
The ceiling isn't a hard protocol limit — it's context-window economics. Every tool schema is tokens the agent spends *every turn*. Thirty tools with rich schemas can eat 3–5k tokens before the conversation even starts.

---

## Errors

Return MCP tool errors, not exceptions that crash the transport. Include enough detail for the agent to recover or retry differently.
