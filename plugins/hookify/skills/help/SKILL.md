...
These hooks read configuration files from `.codeAgent/hookify.*.local.md` and check if any rules match the current operation.

### 2. Configuration Files

Users create rules in `.codeAgent/hookify.{rule-name}.local.md` files:
...
Create `.codeAgent/hookify.my-rule.local.md` with the format above.
...
- **`/help`** - Show this help (what you're reading now)
- **`/list`** - List all configured hooks
- **`/configure`** - Enable/disable existing hooks interactively
...
**Rule Files**: Keep rules in `.codeAgent/hookify.*.local.md` - they should be git-ignored (add to .gitignore if needed).
...
**Hook not triggering:**
- Check rule file is in `.codeAgent/` directory
...
4. Refine the rule by editing `.codeAgent/hookify.warn-rm.local.md`

5. Create more rules as you encounter unwanted behaviors

For more examples, check the `~/.codeAgent/plugins/<插件名>/examples/` directory.
