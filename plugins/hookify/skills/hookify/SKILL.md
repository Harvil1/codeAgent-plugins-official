...
<!-- CodeAgent 适配注：本插件的规则引擎原靠 Claude Code 的 hooks 事件触发；本环境暂不从插件加载 hooks。规则文件仍可读写，但要真正自动触发，需把对应钩子配置进 ~/.codeAgent 的 hooks 设置。 -->
# Hookify - Create Hooks from Unwanted Behaviors

**FIRST: Load the hookify:writing-rules skill** using the load_skill tool to understand rule file format and syntax.
...
**If 用户在命令后敲的参数（见本消息末尾原文） is provided:**
- User has given specific instructions: `用户在命令后敲的参数（见本消息末尾原文）`
...
**If 用户在命令后敲的参数（见本消息末尾原文） is empty:**
...
Use the delegate_task tool to launch conversation-analyzer agent:
```
{
  "subagent_type": "general-purpose",
  "description": "Analyze conversation for unwanted behaviors",
  "prompt": "You are analyzing a CodeAgent conversation to find behaviors the user wants to prevent.
...
For each confirmed behavior, create a `.codeAgent/hookify.{rule-name}.local.md` file:
...
**IMPORTANT**: Rule files must be created in the current working directory's `.codeAgent/` folder, NOT the plugin directory.

Use the current working directory (where CodeAgent was started) as the base path.

1. Check if `.codeAgent/` directory exists in current working directory
   - If not, create it first with: `mkdir -p .the agent`

2. Use write_file tool to create each `.codeAgent/hookify.{name}.local.md` file
   - Use relative path from current working directory: `.codeAgent/hookify.{name}.local.md`
   - The path should resolve to the project's .the agent directory, not the plugin's

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .codeAgent/hookify.dangerous-rm.local.md
   - .codeAgent/hookify.console-log.local.md
   - .codeAgent/hookify.sensitive-files.local.md
...
- **bash**: Matches terminal tool commands
...
4. Create `.codeAgent/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in project's `.codeAgent/` directory (current working directory), NOT the plugin's .codeAgent/
...
2. Ensure `.codeAgent/` directory exists (create with mkdir if needed)
3. Use absolute path if needed: `{cwd}/.codeAgent/hookify.{name}.local.md`
4. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in project `.codeAgent/` not plugin `.codeAgent/`
2. Check file with read_file tool to ensure pattern is correct
...
Use task_create to track your progress through the steps.
