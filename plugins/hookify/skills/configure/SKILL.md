...
```
pattern: ".codeAgent/hookify.*.local.md"
```
...
- Use read_file tool to read current content
- Use str_replace tool to change `enabled: true` to `enabled: false` (or vice versa)
...
- You can also manually edit .codeAgent/hookify.*.local.md files
- To permanently remove a rule, delete its .local.md file
- Use `/list` to see all configured rules
