...
risks / FAQ tabs when they earn their place), published with CodeAgent's built-in
...
- CodeAgent's built-in `Artifact` tool, which requires a claude.ai login (sessions on an
  API key, Bedrock, or Vertex don't have it). CodeAgent Artifacts are available in beta
...
- Artifact URLs are minted by the server. The plugin records yours after the first publish
  ...
  others can find it.
- Publishing needs an interactive session: headless (`the agent -p`) runs don't have the
