...
description: Generate and publish a project status artifact — an opinionated, tabbed status page for a project too big for one update (overview & success criteria, the workstream sequence, next steps, plus background, plan, risks & open questions, and decisions/FAQ when they earn a tab) — published as a self-contained local HTML file the user can open in a browser or share with teammates. Use when a piece of work spans several workstreams and you want a shareable overview kept current. Each artifact is backed by a small per-project config in the plugin data dir, so refreshing it re-gathers live state, redeploys the same URL, and reports only the delta. For software projects whose workstreams are PRs, also read swe.md (the X.Y PR-numbering convention; pulling PR state with gh/git; a per-PR detail block). Generates one self-contained .html file (open directly in a browser). Not for single-PR changes or public docs.
user-invocable: true
---
...
generates one self-contained HTML file (everything inlined; the only `<script>` is the tab
switcher) and writes it to `page.html` in the artifact directory — the publish step is
telling the user that local file path (they can open it in a browser or send the file
to teammates). Version history is kept as `page-<n>.html` snapshots alongside it. (The general "render any HTML/Markdown to a web page" capability is the built-in
...
1. **Resolve the artifact config, then locate the project.** Each project gets a directory
   at `~/.codeAgent/plugins/<插件名>/artifacts/<slug>/` holding `config.md` (see **"The artifact
...
3. **Generate the HTML** from `template.html` in this skill directory (same folder as this
   ...
   `html` path** — default `~/.codeAgent/plugins/<插件名>/artifacts/<slug>/page.html`, next to the
   config (not `/tmp`; not inside the user's repo unless they ask — if they do, use
   `<repo>/.codeAgent/project-artifact/<slug>.html` and record it as the config's `html` path):
...
8. **Write the config and report.** On a first publish, write
   `~/.codeAgent/plugins/<插件名>/artifacts/<slug>/config.md` now — recording the minted URL, favicon,
   ...
   session published a newer version), web_fetch the URL to see the current content,
   reconcile, then publish again.

Headless note: the Artifact tool is not available in non-interactive (`the agent -p`)
...
A small markdown file at `~/.codeAgent/plugins/<插件名>/artifacts/<slug>/config.md`, in the
...
- **Artifact** — `url` (written after the first publish; every later publish passes it),
  `favicon`, `title`, `html` path (default `~/.codeAgent/plugins/<插件名>/artifacts/<slug>/page.html`).
...
- **Read the previous render before overwriting it.** Parse its state block; its `as_of`
  ...
  the config has a `url` (new machine, reinstall), web_fetch the artifact URL to recover
...
**`claude.ai/code/artifact/...`** — use web_fetch with the URL; it returns the page HTML.
