...
A comprehensive example plugin demonstrating CodeAgent extension options.
...
├── skills/
│   ├── example-skill/
│   │   └── SKILL.md           # Model-invoked load_skill (contextual guidance)
│   └── example-command/
│       └── SKILL.md           # User-invoked load_skill (slash command)
└── commands/
...
**Model-invoked load_skill** (activated by task context):
...
**User-invoked load_skill** (slash command — `/skill-name`):
