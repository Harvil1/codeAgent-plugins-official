---
name: example-skill
description: This skill should be used when the user asks to "demonstrate skills", "show skill format", "create a skill template", or discusses skill development patterns. Provides a reference template for creating CodeAgent plugin skills.
version: 1.0.0
---

# Example Skill

This skill demonstrates the structure and format for CodeAgent plugin skills.

## Overview

Skills are model-invoked capabilities that the agent autonomously uses based on task context. Unlike commands (user-invoked) or agents (spawned by the agent), skills provide contextual guidance that the agent incorporates into its responses.
...
- **description** (required): Trigger conditions - describe when the agent should use this skill
- **version** (optional): Semantic version number
- **license** (optional): License information or reference

## Writing Effective Descriptions

The description field is crucial - it tells the agent when to invoke the skill.
