...
Connect the agent to an MCP server running inside your private network through an
...
testing). It walks you through the parts only you can do in the the agent Console
...
7. **Start and verify** — brings the stack up and checks the proxy and tunnel
   logs.
8. **Call it from the agent** — shows you how to reach the server from Managed
...
When it's running, the routed server is reachable from the agent at
...
- A the agent Console role that can manage MCP tunnels.
