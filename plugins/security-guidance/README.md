...
1. **Pattern warnings** — instant regex-based reminders on `str_replace`/`write_file` for ~25 known-dangerous patterns (`yaml.load`, `torch.load(weights_only=False)`, `pickle.load` on untrusted data, raw `innerHTML`, hardcoded secrets, etc.).
2. **LLM diff review** — when the agent finishes a turn, the plugin sends the diff to a fast LLM call (Opus 4.7 by default) and feeds high-severity findings back to the agent so it can fix them before you see the response.
3. **Agentic commit review** — on `git commit`, an SDK-driven reviewer reads related files (`read_file`/`search_files`/`glob`) to trace data flow across the codebase, catching multi-file vulnerabilities pattern matching misses (IDOR, auth bypass, cross-file SSRF).
...
Marketplace ships enabled by default in CodeAgent — no setup beyond having the CLI itself.

## Prerequisites

- CodeAgent CLI ≥ v2.1.144
...
- `~/.codeAgent/claude-security-guidance.md` — user-wide rules
...
The plugin sends data to a model endpoint to perform its reviews. Specifically, each Stop-hook diff review transmits the changed file paths, the diff hunks, and the relevant file contents in the diff; each agentic commit review additionally transmits any files the reviewer pulls in via `read_file`/`search_files`/`glob` while tracing data flow. Your `claude-security-guidance.md` contents (user, project, and local) are appended to the prompt on every review, so don't put secrets in it.

Where that data goes depends on your CodeAgent configuration:
...
The plugin writes its own debug log to `~/.codeAgent/security/log.txt` (override with `SECURITY_GUIDANCE_DEBUG_LOG`). The log contains diffstate metadata and finding categories — no full file contents or model prompts — and rotates at 1 MB. Nothing is uploaded.
...
**Plugin doesn't seem to fire** — check that `~/.codeAgent/claude-security-guidance.md` (or hook activity) shows in debug logs. Run CodeAgent with `--debug-file /tmp/claude/debug.txt` and grep for `security_reminder_hook`. The plugin also writes its own log to `~/.codeAgent/security/log.txt`.
...
- The CodeAgent CLI version (`the agent --version`)
- Provider setup (1P / Bedrock / Vertex / LLM gateway / etc.)
- A minimal repro diff
- The relevant section of `~/.codeAgent/security/log.txt`
