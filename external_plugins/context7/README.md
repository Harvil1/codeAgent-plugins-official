...
This plugin connects CodeAgent to Context7's hosted remote MCP server (`https://mcp.context7.com/mcp`) — no local Node.js, npm, or npx required.
...
Without an API key the plugin connects anonymously and shares the anonymous rate limits. To use your own plan, create an API key in the [Context7 dashboard](https://context7.com/dashboard) and export it as an environment variable before launching CodeAgent:
...
The plugin's MCP server configuration picks up `CONTEXT7_API_KEY` automatically. Restart CodeAgent after setting it, then verify usage in the [dashboard](https://context7.com/dashboard).
