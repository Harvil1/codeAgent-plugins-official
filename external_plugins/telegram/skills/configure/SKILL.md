...
# /configure — Telegram Channel Setup
...
```bash
echo "${TELEGRAM_STATE_DIR:-${CLAUDE_CONFIG_DIR:-$HOME/.the agent}/channels/telegram}"
```

Use the printed path everywhere below in place of `<state-dir>`. The default
is `~/.codeAgent/channels/telegram`.
...
3. **What next** — end with a concrete next step based on state:
   - No token → *"Run `/configure <token>` with the token from
     BotFather."*
   - Token set, policy is pairing, nobody allowed → *"DM your bot on
     Telegram. It replies with a code; approve with `/access pair
     <code>`."*
...
6. **If policy is already `allowlist`** → confirm this is the locked state.
   ...
   `/access policy pairing` → they DM → you pair → flip back."*
...
1. Treat `用户在命令后敲的参数（见本消息末尾原文）` as the token (trim whitespace). BotFather tokens look
