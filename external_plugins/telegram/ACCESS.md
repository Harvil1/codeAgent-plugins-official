...
By default, a DM from an unknown sender triggers **pairing**: the bot replies with a 6-character code and drops the message. You run `/access pair <code>` from your assistant session to approve them. Once approved, their messages pass through.

All state lives in `~/.codeAgent/channels/telegram/access.json`. The `/access` skill commands edit this file; the server re-reads it on every inbound message, so changes take effect without a restart. Set `TELEGRAM_ACCESS_MODE=static` to pin config to what was on disk at boot (pairing is unavailable in static mode since it requires runtime writes).
...
| Config file | `~/.codeAgent/channels/telegram/access.json` |
...
| Policy | Behavior |
| --- | --- |
| `pairing` (default) | Reply with a pairing code, drop the message. Approve with `/access pair <code>`. |
...
```
/access allow 412587349
/access remove 412587349
```
...
```
/access group add -1001654782309
```

Supergroup IDs are negative numbers with a `-100` prefix, e.g. `-1001654782309`. They're not shown in the Telegram UI. To find one, either add [@RawDataBot](https://t.me/RawDataBot) to the group temporarily (it dumps a JSON blob including the chat ID), or add your bot and run `/access` to see recent dropped-from groups.
...
```
/access group add -1001654782309 --no-mention
/access group add -1001654782309 --allow 412587349,628194073
/access group rm -1001654782309
```
...
```
/access set mentionPatterns '["^hey the agent\\b", "\\bassistant\\b"]'
```

## Delivery

Configure outbound behavior with `/access set <key> <value>`.
...
| Command | Effect |
| --- | --- |
| `/access` | Print current state: policy, allowlist, pending pairings, enabled groups. |
| `/access pair a4f91c` | Approve pairing code `a4f91c`. Adds the sender to `allowFrom` and sends a confirmation on Telegram. |
| `/access deny a4f91c` | Discard a pending code. The sender is not notified. |
| `/access allow 412587349` | Add a user ID directly. |
| `/access remove 412587349` | Remove from the allowlist. |
| `/access policy allowlist` | Set `dmPolicy`. Values: `pairing`, `allowlist`, `disabled`. |
| `/access group add -1001654782309` | Enable a group. Flags: `--no-mention` (also requires disabling privacy mode), `--allow id1,id2`. |
| `/access group rm -1001654782309` | Disable a group. |
| `/access set ackReaction 👀` | Set a config key: `ackReaction`, `replyToMode`, `textChunkLimit`, `chunkMode`, `mentionPatterns`. |

## Config file

`~/.codeAgent/channels/telegram/access.json`. Absent file is equivalent to `pairing` policy with empty lists, so the first DM triggers pairing.
...
```jsonc
{
  ...
  "mentionPatterns": ["^hey the agent\\b"],
