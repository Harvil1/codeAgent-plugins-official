...
<!-- CodeAgent 适配注：原 Claude Code 的 allowed-tools（Read/Write/terminal(...)）已移除——
本项目按工具整名精确匹配、不认 CC 写法，保留会把工具表滤空。 -->

# /configure — Discord Channel Setup

Writes the bot token to `~/.codeAgent/channels/discord/.env` and orients the
user on access policy. The server reads both files at boot.

Arguments passed: `用户在命令后敲的参数（见本消息末尾原文）`
...
1. **Token** — check `~/.codeAgent/channels/discord/.env` for
   `DISCORD_BOT_TOKEN`. Show set/not-set; if set, show first 6 chars masked.

2. **Access** — read `~/.codeAgent/channels/discord/access.json` (missing file
...
3. **What next** — end with a concrete next step based on state:
   - No token → *"Run `/configure <token>` with your bot token from
     the Developer Portal → Bot → Reset Token."*
   - Token set, policy is pairing, nobody allowed → *"DM your bot on
     Discord. It replies with a code; approve with `/access pair
     <code>`."*
...
3. **If yes and policy is still `pairing`** → *"Good. Let's lock it down so
   ...
   `/access policy allowlist`. Do this proactively — don't wait to
   be asked.
4. **If no, people are missing** → *"Have them DM the bot; you'll approve
   each with `/access pair <code>`. Run this skill again once
   ...
   right-click them → Copy User ID, then `/access allow <id>`."*
...
- `access.json` is re-read on every inbound message — policy changes via
  `/access` take effect immediately, no restart.
