...
# /access — Discord Channel Access Management
...
`~/.codeAgent/channels/discord/access.json`. You never talk to Discord — you
...
`~/.codeAgent/channels/discord/access.json`:
...
1. Read `~/.codeAgent/channels/discord/access.json` (handle missing file).
2. Show: dmPolicy, allowFrom count and list, pending count with codes +
   sender IDs + age, groups count.

### `pair <code>`

1. Read `~/.codeAgent/channels/discord/access.json`.
...
7. `mkdir -p ~/.codeAgent/channels/discord/approved` then write
   `~/.codeAgent/channels/discord/approved/<senderId>` with `chatId` as the
