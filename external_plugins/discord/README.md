...
Connect a Discord bot to your CodeAgent with an MCP server.

When the bot receives a message, the MCP server forwards it to the agent and provides tools to reply, react, and edit messages.
...
Writes `DISCORD_BOT_TOKEN=...` to `~/.codeAgent/channels/discord/.env`. You can also write that file by hand, or set the variable in your shell environment — shell takes precedence.
...
```sh
uv run python main.py --channels plugin:discord@claude-plugins-official
```

**7. Pair.**

With CodeAgent running from the previous step, DM your bot on Discord — it replies with a pairing code. If the bot doesn't respond, make sure your session is running with `--channels`. In your CodeAgent session:
...
| `download_attachment` | Download all attachments from a specific message by ID to `~/.codeAgent/channels/discord/inbox/`. Returns file paths + metadata. Use when `fetch_messages` shows a message has attachments. |
...
Downloads land in `~/.codeAgent/channels/discord/inbox/`.
