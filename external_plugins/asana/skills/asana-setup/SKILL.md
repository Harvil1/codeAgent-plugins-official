---
name: asana-setup
description: Set up the Asana V2 MCP server connection (one-time OAuth app + the agent mcp add)
---


The user wants to connect CodeAgent to Asana's V2 MCP server. Guide them through the one-time setup below. Do NOT run `the agent mcp add` yourself — the `--client-secret` prompt needs a real terminal (a hidden TTY prompt), so the user must run it in their own terminal.
...
Note: `localhost` is correct — CodeAgent is a local client and catches the OAuth callback on your own machine. Asana's V2 server does not support Dynamic Client Registration, so you must bring your own client_id + client_secret.
...
1. On first use, CodeAgent opens your browser for Asana consent — approve it.
