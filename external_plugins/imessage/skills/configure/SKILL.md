...
<!-- CodeAgent 适配注：原 Claude Code 的 allowed-tools（Read/terminal(...)）已移除——
本项目按工具整名精确匹配、不认 CC 写法，保留会把工具表滤空。 -->

# /configure — iMessage Channel Setup
...
Arguments passed: `用户在命令后敲的参数（见本消息末尾原文）` (unused — this skill only shows status)
...
1. **Full Disk Access** — run `ls ~/Library/Messages/chat.db`. If it fails
   ...
   Access to your terminal (or IDE if that's where CodeAgent runs): System
   Settings → Privacy & Security → Full Disk Access. The server can't read
   chat.db without it."*

2. **Access** — read `~/.codeAgent/channels/imessage/access.json` (missing file
...
3. **What next** — end with a concrete next step based on state:
   ...
   - FDA granted, policy is allowlist → *"Text yourself from any device
     ...
     someone else through: `/access allow +15551234567`."*
...
4. **"My partner / a friend / a couple people"** → ask for each handle
   ...
   run `/access allow <handle>` for each. Stay on `allowlist`.
5. **Current policy is `pairing`** → flag it immediately: *"Your policy is
   ...
   Switch back to `allowlist`?"* and offer `/access policy
