...
`~/.codeAgent/channels/imessage/access.json`:
...
Parse `用户在命令后敲的参数（见本消息末尾原文）` (space-separated). If empty or unrecognized, show status.

### No args — status

1. Read `~/.codeAgent/channels/imessage/access.json` (handle missing file).
2. Show: dmPolicy, allowFrom count and list, pending count with codes +
   sender IDs + age, groups count.

### `pair <code>`

1. Read `~/.codeAgent/channels/imessage/access.json`.
...
7. `mkdir -p ~/.codeAgent/channels/imessage/approved` then write
   `~/.codeAgent/channels/imessage/approved/<senderId>` with `chatId` as the
