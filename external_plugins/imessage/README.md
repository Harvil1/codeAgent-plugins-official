...
Connect iMessage to your CodeAgent assistant. Reads `~/Library/Messages/chat.db` directly for history, search, and new-message detection; sends via AppleScript to Messages.app. No external server, no background process to keep alive.
...
```sh
uv run python main.py --channels plugin:imessage@claude-plugins-official
```

Check that `/configure` tab-completes.
...
Handles are phone numbers (`+15551234567`) or Apple ID emails (`them@icloud.com`). If you're not sure what you want, ask the agent to review your setup.
...
| **Attachments** | `chat.db` stores absolute filesystem paths. The first inbound image per message is surfaced to the assistant as a local path it can `read_file`. Outbound attachments send as separate messages after the text. |

## Environment variables

| Variable | Default | Effect |
| --- | --- | --- |
| `IMESSAGE_APPEND_SIGNATURE` | `true` | Appends `\nSent by the agent` to outbound messages. Set to `false` to disable. |
...
| `IMESSAGE_STATE_DIR` | `~/.codeAgent/channels/imessage` | Override where `access.json` and pairing state live. |
